...
TEST_DATASET_PATH=dataset/clef2023/${LANGUAGE}/test_tr_gold.tsv

#for PRETRAINED_MODEL in "xlm-roberta-base" "dbmdz/bert-base-turkish-128k-cased"; do
for PRETRAINED_MODEL in "dbmdz/bert-base-turkish-128k-cased"; do
    echo Experiment
    echo $MODEL
...
#         #   for AUGMENT in normal duygusal propaganda öznel all ; do
          for AUGMENT in normal duygusal propaganda öznel abartılı aşağılayıcı partizan önyargılı all; do
#           for AUGMENT in all; do
               ...
               MODEL_PATH=saved_model/${PRETRAINED_MODEL}/${LANGUAGE}/${AUGMENT}
               AUGMENT_DATASET_PATH=dataset/augmented_data/clef2023/chat-gpt/${LANGUAGE}/${AUGMENT}.tsv
  
               mkdir -p $MODEL_PATH
  
               RESULT_OUTPUT=results/${PRETRAINED_MODEL}/${LANGUAGE}/chat-gpt/${AUGMENT}/${AUGMENT_TYPE}
